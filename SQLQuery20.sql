...
-- This prompts SQL that a procedure or function will be stored with the database concerning the actions below
	SELECT VendorName, VendorState, VendorPhone
	-- columns used
	FROM Vendors
	-- table used
	WHERE VendorState = @StateVar
	-- condition in which VendorState is equal to @StateVar
	ORDER BY VendorName;
	-- order by VendorName
-- This is a statement that executes the spVendorByState stored procedure
EXEC spVendorsByState 'CA';
-- CA is the VendorState given to the procedure to look up
/*
